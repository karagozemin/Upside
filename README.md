...
| *"real SoDEX testnet execution ... would make this top-tier"* — **Davislambo** | EIP-712 signing path + judge-safe fallback, real order-ID surface | Step 4 + `/desk/monitor` |
...
When signing keys are absent: full UI completes with honest **judge-safe simulated** disclosure — no fake order IDs.
...
- [ ] Real SoDEX testnet order with deposited API keys (path is live; needs funded key)
