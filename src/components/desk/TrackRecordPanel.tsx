  ...
  return (
    <div className="panel overflow-hidden">
      <div className="border-b border-white/5 px-5 py-4">
        ...
        <p className="mt-1 text-xs text-[#64748b]">Outcome tracking for protection recommendations</p>
      </div>
      <div className="divide-y divide-white/5">
        {DEMO_TRACK_RECORD.map((e) => (
          <div key={e.id} className="flex items-center justify-between gap-4 px-5 py-3 text-sm">
            <div>
              <p className="mono text-[10px] text-[#64748b]">{e.id}</p>
              <p className="font-medium">{e.asset} · {e.riskBefore} → {e.riskAfter}</p>
              <p className="text-xs text-[#64748b]">{e.action} — {e.note}</p>
            </div>
